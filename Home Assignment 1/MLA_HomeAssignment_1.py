...
#     # test_label = training_labels[test_label]
#     distances = np.sum((training_points - test_point)**2, axis=1)
#     sorted_indices = np.argsort(distances)
#     cumulative_labels = np.cumsum(training_labels[sorted_indices])
#     errors_for_K = np.not_equal(cumulative_labels, test_label)
#     return errors_for_K
...
# Calculate errors for the specific test point
...
# # Initialize validation sets and n values
# m = 50  # Number of training points
# n_values = [10, 20, 40, 80]

# # Store validation errors for different n values
# validation_errors = []
...
#                 num_errors += errors[k-1]
            
#             validation_errors_for_set.append(num_errors / len(validation_indices))
        
#         validation_errors_for_n.append(validation_errors_for_set)
    
#     validation_errors.append(validation_errors_for_n)

# # Plot validation error curves for different n values
# for n_idx, validation_errors_for_n in enumerate(validation_errors):
#     n = n_values[n_idx]
    
#     fig, ax = plt.subplots(figsize=(8, 5))
    
#     for i, validation_error_curve in enumerate(validation_errors_for_n, start=1):
#         ax.plot(range(1, m + 1), validation_error_curve, label=f"Validation Set {i}")
    
#     ax.set_xlabel("K")
#     ax.set_ylabel("Validation Error")
#     ax.set_title(f"Validation Error as a Function of K for n = {n}")
#     ax.grid(alpha=0.2)
#     ax.legend()
#     plt.show()